...
class GeneralAssemblyRepository(object):
    ...
    @classmethod
    def add_general_assembly(cls, general_assembly):
        """
        Add a general assembly to the repository

        Args:
            general_assembly: The general assembly business entity to be added
                to the repository.
        """
        assembly = GeneralAssembly.create_from_business_entity(
            general_assembly)
    ...
    @classmethod
    def update_general_assembly(cls, general_assembly):
        """
        Update a general assembly

        The general assembly is identified by its number as the business key.

        Args:
            general_assembly: The general assembly business entity to be
                updated in the repository.

        Raises:
            ValueError: In case the a general assembly does not exist.
        """
        assembly = cls.get_general_assembly(general_assembly.number)
        if assembly is None:
            raise ValueError(
                'A general assembly with this number does not exist.')
        assembly.update_from_business_entity(general_assembly)
        # pylint: disable=no-member
        DBSession.flush()
    ...
    @classmethod
    def get_member_invitations(cls,
                               membership_number,
                               earliest=None,
                               latest=None):
        """
        ...
        if earliest is not None:
            assembly_date_filter = and_(assembly_date_filter,
        ...
        assemblies = (
            ...
            # sent date and token
            DBSession.query(
                GeneralAssembly.number, GeneralAssembly.name,
                GeneralAssembly.date, GeneralAssemblyInvitation.sent,
                GeneralAssemblyInvitation.token).select_from(GeneralAssembly)
    ...
    def get_member_by_token(cls, token):
        ...
        # pylint: disable=no-member
        return (
            DBSession.query(C3sMember).join(GeneralAssemblyInvitation).filter(
                GeneralAssemblyInvitation.token == token).first())
