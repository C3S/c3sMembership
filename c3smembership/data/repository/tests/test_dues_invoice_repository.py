...
class TestDuesInvoiceRepository(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with transaction.manager:
            ...
            member1.dues21_paid_date = date(2021, 11, 21)
            member1.dues21_amount_paid = Decimal('21.11')
            member1.dues22_paid_date = date(2022, 11, 22)
            member1.dues22_amount_paid = Decimal('22.11')
            member1.dues23_paid_date = date(2023, 11, 23)
            ...
            self.db_session.add(
                Dues23Invoice(invoice_no=2023,
                              ...
                              token='23WXYZ7890'))
            self.db_session.add(
                Dues24Invoice(invoice_no=2024,
                              invoice_no_string='dues24-1234',
                              invoice_date=date(2024, 2, 24),
                              invoice_amount=Decimal('1234.24'),
                              member_id=member1.id,
                              membership_no=member1.membership_number,
                              email=member1.email,
                              token='24WXYZ7890'))
            self.db_session.add(
                Dues25Invoice(invoice_no=2025,
                              invoice_no_string='dues25-1234',
                              invoice_date=date(2025, 2, 25),
                              invoice_amount=Decimal('1234.25'),
                              member_id=member1.id,
                              membership_no=member1.membership_number,
                              email=member1.email,
                              token='25WXYZ7890'))
    ...
    def test_store_dues(self):
        ...
        # 2024
        dues_calculation = DuesCalculation(Decimal('50.0'), 'q1_2024')

        DuesInvoiceRepository.store_dues(2024, member, dues_calculation)

        self.assertEqual(member.dues24_amount, Decimal('50.0'))
        self.assertEqual(member.dues24_start, 'q1_2024')

        # 2025
        dues_calculation = DuesCalculation(Decimal('50.0'), 'q1_2025')

        DuesInvoiceRepository.store_dues(2025, member, dues_calculation)
    ...
    def test_record_dues_email_sent(self):
        ...
        # 2021
        member.dues21_invoice = None
        member.dues21_invoice_date = None

        DuesInvoiceRepository.record_dues_email_sent(2021, member)

        self.assertTrue(member.dues21_invoice)
        self.assertEqual(member.dues21_invoice_date.date(), date.today())

        # 2022
        member.dues22_invoice = None
        member.dues22_invoice_date = None

        DuesInvoiceRepository.record_dues_email_sent(2022, member)

        self.assertTrue(member.dues22_invoice)
        self.assertEqual(member.dues22_invoice_date.date(), date.today())

        # 2023
        member.dues23_invoice = None
        member.dues23_invoice_date = None

        DuesInvoiceRepository.record_dues_email_sent(2023, member)

        self.assertTrue(member.dues23_invoice)
        self.assertEqual(member.dues23_invoice_date.date(), date.today())

        # 2024
        member.dues24_invoice = None
        member.dues24_invoice_date = None

        DuesInvoiceRepository.record_dues_email_sent(2024, member)

        self.assertTrue(member.dues24_invoice)
        self.assertEqual(member.dues24_invoice_date.date(), date.today())

        # 2025
        member.dues25_invoice = None
        member.dues25_invoice_date = None

        DuesInvoiceRepository.record_dues_email_sent(2025, member)
