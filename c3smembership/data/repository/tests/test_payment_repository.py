...
class TestPaymentRepository(unittest.TestCase):
    ...
    def test_get_payments(self):
        ...
        2. Test first payment for sorting
        3. Test paging and 4th payment for sorting
        4. Test 2nd records for sorting
        5. Test 7th record for sorting
        6. Test filtering
        """
        ...
        payments = PaymentRepository.get_payments(
            1, 100, date(2016, 1, 31), date(2017, 3, 1))
        self.assertEqual(len(payments), 4)

        payments = PaymentRepository.get_payments(
            1, 100, date(2016, 2, 1), date(2017, 3, 1))
        self.assertEqual(len(payments), 4)

        payments = PaymentRepository.get_payments(
            1, 100, date(2016, 2, 2), date(2017, 3, 1))
        self.assertEqual(len(payments), 3)

        payments = PaymentRepository.get_payments(
            1, 100, date(2016, 2, 2), date(2017, 3, 5))
        self.assertEqual(len(payments), 4)
