...
class Dues19Invoice(Base):
    """
    This table stores the invoices for the 2019 version of dues.
    ...
    @classmethod
    def check_for_existing_dues19_token(cls, dues_token):
        """
    ...
    def get_monthly_stats(cls):
        ...
        #     'month',
        #     Dues19Invoice.invoice_date)
        invoice_date_month = func.substr(Dues19Invoice.invoice_date, 1, 7)
        payment_date_month = func.substr(C3sMember.dues19_paid_date, 1, 7)
        ...
        invoice_amounts_query = DBSession.query(
            ...
            func.sum(expression.case(
                [(
                    expression.not_(Dues19Invoice.is_reversal),
                    Dues19Invoice.invoice_amount)],
                else_=Decimal('0.0'))).label('amount_invoiced_normal'),
            func.sum(expression.case(
                [(
                    Dues19Invoice.is_reversal,
                    Dues19Invoice.invoice_amount)],
        ...
        member_payments_query = DBSession.query(
            ...
            func.sum(C3sMember.dues19_amount_paid).label('amount_paid')
        ).filter(C3sMember.dues19_paid_date.isnot(None)) \
