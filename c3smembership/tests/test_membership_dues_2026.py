...
class TestDues26Views(unittest.TestCase):
    """
    ...
    def test_send_dues26_invoice_email_single(self):
        """
        test the send_dues26_invoice_email view
        ...
        * english investing legal entity
        ...
        from c3smembership.presentation.views.dues_2026 import (
            send_dues26_invoice_email,
        )

        _number_of_invoices = len(DuesInvoiceRepository.get_all([2026]))

        self.config.add_route('dues', '/')
        self.config.add_route('detail', '/')
        self.config.add_route('membership_listing_backend', '/')
        self.config.add_route('make_dues26_invoice_no_pdf', '/')
        ...
        _number_of_invoices_3 = len(DuesInvoiceRepository.get_all([2026]))
        ...
        self.assertTrue(
            'Verwendungszweck: C3S-dues2026-0001' in mailer.outbox[0].body)
        ...
        res3 = send_dues26_invoice_email(req3)
        self.assertTrue(res3.status_code == 302)
        self.assertTrue('http://example.com/' in res3.headers['Location'])
        _number_of_invoices_4 = len(DuesInvoiceRepository.get_all([2026]))
        ...
        * legal entities (de/en)
        ...
        res_en_normal = send_dues26_invoice_email(req_en_normal)
        ...
        res_de_investing = send_dues26_invoice_email(req_de_investing)
        ...
        res_en_investing = send_dues26_invoice_email(req_en_investing)
        ...
        res_de_legalentity = send_dues26_invoice_email(req_de_legalentity)
        ...
        res_en_legalentity = send_dues26_invoice_email(req_en_legalentity)
    ...
    def test_send_dues26_invoice_email_via_BATCH(self):
        """
        test the send_dues26_invoice_batch function
        for batch processing
        """
        # from pyramid_mailer import get_mailer
        from c3smembership.presentation.views.dues_2026 import (
            send_dues26_invoice_batch,
        )
        self.config.add_route('make_dues26_invoice_no_pdf', '/')
        self.config.add_route('make_dues26_reversal_invoice_pdf', '/')
        ...
        _number_of_invoices_before_batch = len(
            DuesInvoiceRepository.get_all([2026]))
        assert _number_of_invoices_before_batch == 0

        req = testing.DummyRequest()
        req.referer = 'toolbox'
        res = send_dues26_invoice_batch(req)

        # check number of invoices: should be 2
        _number_of_invoices_batch = len(DuesInvoiceRepository.get_all([2026]))
        ...
        res = send_dues26_invoice_batch(req_post)
        ...
        res2 = send_dues26_invoice_batch(req)
        ...
        """
        and now some tests for make_dues26_invoice_no_pdf
        """
        from c3smembership.presentation.views.dues_2026 import (
        ...
        req2.matchdict = {
            'email': m1.email,
            'code': m1.dues26_token + 'false!!!',  # must fail
            'i': '0001',
        }

        res = make_dues26_invoice_no_pdf(req2)
        ...
        i2 = DuesInvoiceRepository.get_by_number(2, 2026)
        i2.token = 'not_matching'
        req2.matchdict = {
            'email': m2.email,
            'code': m2.dues26_token,
            'i': '3',  # must fail
        }
        res = make_dues26_invoice_no_pdf(req2)
        assert 'application/pdf' not in res.headers['Content-Type']  # no PDF
        ...
        i1 = DuesInvoiceRepository.get_by_number(1, 2026)
        ...
        test dues listing
        """
        from c3smembership.presentation.views.dues_2026 import dues26_listing
        req_list = testing.DummyRequest()
        resp_list = dues26_listing(req_list)
        assert resp_list['count'] == 2

    def test_dues26_reduction(self):
        """
        test the dues26_reduction functionality
        """
        # have to accept their membersip first
        m1 = C3sMember.get_by_id(1)  # german normal member
        m1.membership_accepted = True
        ...
        """
        # pre-check
        self.assertFalse(m1.dues26_reduced)  # not reduced yet!
        _m1_amount_reduced = m1.dues26_amount_reduced  # is Decimal('0')
        _number_of_invoices_before_reduction = len(
            DuesInvoiceRepository.get_all([2026]))
        # we have 2 invoices as of now
        self.assertEqual(len(DuesInvoiceRepository.get_all([2026])), 2)
        # import the function under test
        from c3smembership.presentation.views.dues_2026 import dues26_reduction
        ...
        res_reduce = dues26_reduction(req_reduce)  # call reduce on her
        ...
        res_reduce = dues26_reduction(req_reduce)
        # no new invoice
        self.assertEqual(len(DuesInvoiceRepository.get_all([2026])), 2)
        ...
        assert _m1_amount_reduced != m1.dues26_amount_reduced  # changed!
        assert m1.dues26_amount_reduced == 42  # changed to 42!

        # check the invoice created
        _rev_inv = DuesInvoiceRepository.get_by_number(
            _number_of_invoices_before_reduction + 1, 2026)
        _new_inv = DuesInvoiceRepository.get_by_number(
            _number_of_invoices_before_reduction + 2, 2026)
        assert _rev_inv.invoice_amount == D('-50')
        assert _new_inv.invoice_amount == D('42')

        # we have 4 invoices as of now
        self.assertEqual(len(DuesInvoiceRepository.get_all([2026])), 4)
        ...
        res = make_dues26_reversal_invoice_pdf(req2)
        assert 'application/pdf' not in res.headers['Content-Type']  # no PDF
        assert 'error' in res.headers['Location']  # but error
        ...
        i2.token = m2.dues26_token  # we give it a valid token
        req2.matchdict = {
            'email': m2.email,
            'code': m2.dues26_token,
            'no': '0002',
    ...
    def test_dues26_notice(self):
        """
        test the dues26 notice view -- acknowledge incoming payments
        """
        self.config.add_route('detail', '/detail/')
        self.config.add_route('make_dues26_invoice_no_pdf', '/')
        ...
        send_dues26_invoice_email(req0)
        ...
        from c3smembership.presentation.views.dues_2026 import dues26_notice
        res1 = dues26_notice(req1)
        res1  # tame flymake

        # After the partial payment, some amount has been paid
        self.assertEqual(m1.dues26_paid, True)
        # it has been the partial payment amount
        self.assertEqual(m1.dues26_amount_paid, partial_payment_amount)
        self.assertEqual(m1.dues26_paid_date,
                         datetime(2026, 9, 11, 0, 0))
        # the balance is the original amount subtracted by the partial payment
        self.assertEqual(
            m1.dues26_balance, D(m1.dues26_amount) - partial_payment_amount)
        # and the account is not balanced.
        self.assertEqual(m1.dues26_balanced, False)

        # here comes the request to test
        req2 = testing.DummyRequest(
            matchdict={'member_id': 1},
            POST={
                'amount': D(m1.dues26_amount) - partial_payment_amount,
                'payment_date': '2026-09-13',
            }
        )
        res2 = dues26_notice(req2)
        res2  # tame flymake

        # After the final payment, some amount has been paid
        self.assertEqual(m1.dues26_paid, True)
        # it has been the full amount
        self.assertEqual(m1.dues26_amount_paid, D('50'))
        self.assertEqual(m1.dues26_paid_date,
                         datetime(2026, 9, 13, 0, 0))
        # the balance is 0
        self.assertEqual(m1.dues26_balance, D('0'))
        # and the account is balanced.
        self.assertEqual(m1.dues26_balanced, True)
