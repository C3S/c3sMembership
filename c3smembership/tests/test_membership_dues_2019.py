# -*- coding: utf-8 -*-
"""
This module holds some Unit Tests for the 2019 Membership Dues functionality.
...
class TestDues19Views(unittest.TestCase):
    """
    Basic tests for the views concerning membership dues (2019 edition)
    """
    def setUp(self):
    ...
    def test_send_dues19_invoice_email_single(self):
        ...
        * english investing legal entity
        ...
        from c3smembership.presentation.views.dues_2019 import (
            send_dues19_invoice_email,
        )

        _number_of_invoices = len(Dues19Invoice.get_all())

        self.config.add_route('dues', '/')
        self.config.add_route('detail', '/')
        self.config.add_route('make_dues19_invoice_no_pdf', '/')
        ...
        _number_of_invoices_2 = len(Dues19Invoice.get_all())
        ...
        _number_of_invoices_3 = len(Dues19Invoice.get_all())
        ...
        self.assertTrue(
            'Verwendungszweck: C3S-dues2019-0001' in mailer.outbox[0].body)
        ...
        res2 = send_dues19_invoice_email(req2)
        ...
        * legal entities (de/en)
        ...
        res_en_normal = send_dues19_invoice_email(req_en_normal)
        ...
        res_de_investing = send_dues19_invoice_email(req_de_investing)
        ...
        res_en_investing = send_dues19_invoice_email(req_en_investing)
        ...
        res_de_legalentity = send_dues19_invoice_email(req_de_legalentity)
        ...
        res_en_legalentity = send_dues19_invoice_email(req_en_legalentity)
    ...
    def test_send_dues19_invoice_email_via_BATCH(self):
        """
        test the send_dues19_invoice_batch function
        for batch processing
        """
        # from pyramid_mailer import get_mailer
        from c3smembership.presentation.views.dues_2019 import (
            send_dues19_invoice_batch,
        )
        self.config.add_route('make_dues19_invoice_no_pdf', '/')
        self.config.add_route('make_dues19_reversal_invoice_pdf', '/')
        ...
        _number_of_invoices_before_batch = len(Dues19Invoice.get_all())
        assert(_number_of_invoices_before_batch == 0)

        req = testing.DummyRequest()
        req.referrer = 'toolbox'
        res = send_dues19_invoice_batch(req)

        # check number of invoices: should be 2
        _number_of_invoices_batch = len(Dues19Invoice.get_all())
        ...
        req2.matchdict = {
            'email': m1.email,
            'code': m1.dues19_token + 'false!!!',  # must fail
            'i': u'0001',
        }
        ...
        i2 = Dues19Invoice.get_by_invoice_no(2)
        i2.token = u'not_matching'
        req2.matchdict = {
            'email': m2.email,
            'code': m2.dues19_token,
            'i': u'3',  # must fail
        }
        res = make_dues19_invoice_no_pdf(req2)
        assert('application/pdf' not in res.headers['Content-Type'])  # no PDF
        ...
        i1 = Dues19Invoice.get_by_invoice_no(1)
        ...
        test dues listing
        """
        from c3smembership.presentation.views.dues_2019 import dues19_listing
        req_list = testing.DummyRequest()
        resp_list = dues19_listing(req_list)
        # {'count': 5,
        #   'invoices': [
        #       <c3smembership.models.Dues19Invoice object at 0x7f95df761a50>,
        #       <c3smembership.models.Dues19Invoice object at 0x7f95df761690>,
        #       <c3smembership.models.Dues19Invoice object at 0x7f95df819a50>,
    ...
    def test_dues19_reduction(self):
        """
        test the dues19_reduction functionality
        """
        # have to accept their membersip first
        m1 = C3sMember.get_by_id(1)  # german normal member
        m1.membership_accepted = True
        ...
        _number_of_invoices_before_reduction = len(Dues19Invoice.get_all())
        # we have 2 invoices as of now
        self.assertEqual(len(Dues19Invoice.get_all()), 2)
        # import the function under test
        from c3smembership.presentation.views.dues_2019 import dues19_reduction
        ...
        self.assertEqual(len(Dues19Invoice.get_all()), 2)  # no new invoice
        ...
        _number_of_invoices_after_reduction = len(Dues19Invoice.get_all())
    ...
    def test_dues19_notice(self):
        """
        test the dues19 notice view -- acknowledge incoming payments
        """
        self.config.add_route('detail', '/detail/')
        self.config.add_route('make_dues19_invoice_no_pdf', '/')
        ...
        resp0 = send_dues19_invoice_email(req0)
        resp0  # tame flymake
        ...
        from c3smembership.presentation.views.dues_2019 import dues19_notice
        res1 = dues19_notice(req1)
        res1  # tame flymake

        # After the partial payment, some amount has been paid
        self.assertEqual(m1.dues19_paid, True)
        # it has been the partial payment amount
        self.assertEqual(m1.dues19_amount_paid, partial_payment_amount)
        self.assertEqual(m1.dues19_paid_date,
                         datetime(2019, 9, 11, 0, 0))
        # the balance is the original amount subtracted by the partial payment
        self.assertEqual(
            m1.dues19_balance, D(m1.dues19_amount) - partial_payment_amount)
        # and the account is not balanced.
        self.assertEqual(m1.dues19_balanced, False)

        # here comes the request to test
        req2 = testing.DummyRequest(
            matchdict={'member_id': 1},
            POST={
                'amount': D(m1.dues19_amount) - partial_payment_amount,
                'payment_date': '2019-09-13',
            }
        )
        res2 = dues19_notice(req2)
        res2  # tame flymake

        # After the final payment, some amount has been paid
        self.assertEqual(m1.dues19_paid, True)
        # it has been the full amount
        self.assertEqual(m1.dues19_amount_paid, D('50'))
        self.assertEqual(m1.dues19_paid_date,
                         datetime(2019, 9, 13, 0, 0))
        # the balance is 0
        self.assertEqual(m1.dues19_balance, D('0'))
        # and the account is balanced.
        self.assertEqual(m1.dues19_balanced, True)
