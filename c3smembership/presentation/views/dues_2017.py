...
def send_dues17_invoice_email(request, m_id=None):
    ...
    if member.dues17_invoice is True:
        ...
        while DuesInvoiceRepository.token_exists(randomstring, 2017):
        ...
        try:
            ...
            max_invoice_no = DuesInvoiceRepository.get_max_invoice_number(2017)
...
@view_config(
    ...
    permission='manage')
def make_dues17_invoice_pdf_backend(request):
    """
...
@view_config(
    route_name='dues17_reversal_pdf_backend',
    permission='manage')
def make_dues17_reversal_pdf_backend(request):
    """
    Show the invoice to a backend user
    """
    template = request.registry.settings['c3smembership.certificate_template']
...
def make_invoice_pdf_pdflatex(invoice, template):
    ...
    tex_vars = {
        ...
        'account': str(-dues15_balance - dues16_balance - dues17_balance),
...
@view_config(
    ...
    permission='manage',
    renderer='c3smembership.presentation:templates/pages/dues17_list.pt'
)
def dues17_listing(request):
    ...
    """
    dues17_invoices = DuesInvoiceRepository.get_all([2017])
    return {
