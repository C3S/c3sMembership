...
def string_start_quarter_dues16(member):
    ...
    if 'q1_2016' in member.dues16_start:  # first quarter of 2016 or earlier
        result = "ab Quartal 1" if 'de' in loc else "from 1st quarter"
...
def send_dues16_invoice_email(request, m_id=None):
    ...
    if member.dues16_invoice is True:
        ...
        try:
            ...
            max_invoice_no = DuesInvoiceRepository.get_max_invoice_number(2016)
        ...
        if 'normal' in member.membership_type:  # only for normal members
            ...
            invoice = Dues16Invoice(
                invoice_no=member.dues16_invoice_no,
                invoice_no_string=(
                    'C3S-dues2016-' + str(member.dues16_invoice_no).zfill(4)),
                invoice_date=member.dues16_invoice_date,
                invoice_amount='' + str(member.dues16_amount),
                member_id=member.id,
...
@view_config(
    ...
    permission='manage',
    renderer='c3smembership.presentation:templates/pages/dues16_list.pt'
)
def dues16_listing(request):
    ...
    """
    dues16_invoices = DuesInvoiceRepository.get_all([2016])
    return {
...
def dues16_reduction(request):
    ...
    * issue a new invoice with the new amount (if new amount != 0)
    ...
    reversal_invoice = Dues16Invoice(
        ...
        invoice_no_string=(
            'C3S-dues2016-' + str(new_invoice_no).zfill(4)) + '-S',
    ...
    if not is_exemption:
        ...
        new_invoice = Dues16Invoice(
            ...
            invoice_no_string=(
                'C3S-dues2016-' + str(new_invoice_no + 1).zfill(4)),
            invoice_date=datetime.today(),
    ...
    try:
        ...
        request.session.flash(
            ("Invalid date for payment: '{}' "
             "Use YYYY-MM-DD, e.g. '2016-09-11'".format(
