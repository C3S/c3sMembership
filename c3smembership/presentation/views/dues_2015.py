    ...
    if member.dues15_invoice is True:
        ...
        if 'normal' in member.membership_type:  # only for normal members
            ...
            invoice = Dues15Invoice(
                invoice_no=member.dues15_invoice_no,
                invoice_no_string=(
                    'C3S-dues2015-' + str(member.dues15_invoice_no).zfill(4)),
                invoice_date=member.dues15_invoice_date,
                invoice_amount='' + str(member.dues15_amount),
                member_id=member.id,
...
@view_config(
    ...
    permission='manage',
    renderer='c3smembership.presentation:templates/pages/dues15_list.pt'
)
...
def dues15_reduction(request):
    ...
    * issue a new invoice with the new amount (if new amount != 0)
    ...
    if (member.dues15_reduced and
            ...
            reduced_amount > member.dues15_amount):
        request.session.flash(
    ...
    reversal_invoice = Dues15Invoice(
        ...
        invoice_no_string=(
            'C3S-dues2015-' + str(new_invoice_no).zfill(4)) + '-S',
    ...
    if not is_exemption:
        ...
        new_invoice = Dues15Invoice(
            ...
            invoice_no_string=(
                'C3S-dues2015-' + str(new_invoice_no + 1).zfill(4)),
            invoice_date=datetime.today(),
    ...
    try:
        ...
        request.session.flash(
            ("Invalid date for payment: '{}' "
             "Use YYYY-MM-DD, e.g. '2015-09-11'".format(
