...
def get_member_details(request, member):
    ...
    invoices25 = DuesInvoiceRepository.get_by_membership_number(
        member.membership_number, [2025])
    invoices26 = DuesInvoiceRepository.get_by_membership_number(
        member.membership_number, [2026])
    ...
    if (member.membership_date < date(2026, 12, 31) and
            (
                member.membership_loss_date is None or
                member.membership_loss_date >= date(2026, 1, 1))):
        dues.append({
            'year': '2026',
            'year_short': '26',
            'invoices': invoices26,
            'email_sent': member.dues26_invoice,
            'email_sent_timestamp': member.dues26_invoice_date,
            'has_invoice': len(invoices26) > 0,
            'dues_start': member.dues26_start,
            'dues_amount': member.dues26_amount,
            'is_reduced': member.dues26_reduced,
            'reduced_amount': member.dues26_amount_reduced,
            'is_balanced': member.dues26_balanced,
            'amount_paid': member.dues26_amount_paid,
            'payment_received': member.dues26_paid,
            'paid_date': member.dues26_paid_date,
            'send_email_route': request.route_url(
                'send_dues26_invoice_email', member_id=member.id),
            'reduction_route': request.route_url(
                'dues26_reduction', member_id=member.id),
            'invoice_listing_route': request.route_url('dues26_listing'),
            'dues_notice_route': request.route_url(
                'dues26_notice', member_id=member.id),
            'dues_invoice_pdf_backend': 'dues26_invoice_pdf_backend',
            'dues_reversal_pdf_backend': 'dues26_reversal_pdf_backend',
            'dues_notice_message_to_staff': 'dues26notice_message_to_staff',
        })
    if (member.membership_date < date(2025, 12, 31) and
            (
                member.membership_loss_date is None or
                member.membership_loss_date >= date(2025, 1, 1))):
        dues.append({
            'year': '2025',
            'year_short': '25',
            'invoices': invoices25,
            'email_sent': member.dues25_invoice,
            'email_sent_timestamp': member.dues25_invoice_date,
            'has_invoice': len(invoices25) > 0,
            'dues_start': member.dues25_start,
            'dues_amount': member.dues25_amount,
            'is_reduced': member.dues25_reduced,
            'reduced_amount': member.dues25_amount_reduced,
            'is_balanced': member.dues25_balanced,
            'amount_paid': member.dues25_amount_paid,
            'payment_received': member.dues25_paid,
            'paid_date': member.dues25_paid_date,
            'send_email_route': request.route_url(
                'send_dues25_invoice_email', member_id=member.id),
            'reduction_route': request.route_url(
                'dues25_reduction', member_id=member.id),
            'invoice_listing_route': request.route_url('dues25_listing'),
            'dues_notice_route': request.route_url(
                'dues25_notice', member_id=member.id),
            'dues_invoice_pdf_backend': 'dues25_invoice_pdf_backend',
            'dues_reversal_pdf_backend': 'dues25_reversal_pdf_backend',
            'dues_notice_message_to_staff': 'dues25notice_message_to_staff',
        })
    ...
    if (member.membership_date < date(2022, 12, 31) and
            (
                member.membership_loss_date is None or
                member.membership_loss_date >= date(2022, 1, 1))):
        dues.append({
            'year': '2022',
            'year_short': '22',
            'invoices': invoices22,
            'email_sent': member.dues22_invoice,
            'email_sent_timestamp': member.dues22_invoice_date,
            'has_invoice': len(invoices22) > 0,
            'dues_start': member.dues22_start,
            'dues_amount': member.dues22_amount,
            'is_reduced': member.dues22_reduced,
            'reduced_amount': member.dues22_amount_reduced,
            'is_balanced': member.dues22_balanced,
            'amount_paid': member.dues22_amount_paid,
            'payment_received': member.dues22_paid,
            'paid_date': member.dues22_paid_date,
            'send_email_route': request.route_url(
                'send_dues22_invoice_email', member_id=member.id),
            'reduction_route': request.route_url(
                'dues22_reduction', member_id=member.id),
            'invoice_listing_route': request.route_url('dues22_listing'),
            'dues_notice_route': request.route_url(
                'dues22_notice', member_id=member.id),
            'dues_invoice_pdf_backend': 'dues22_invoice_pdf_backend',
            'dues_reversal_pdf_backend': 'dues22_reversal_pdf_backend',
