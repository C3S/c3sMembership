...
def send_dues19_invoice_email(request, m_id=None):
    ...
    if member.dues19_invoice is True:
        ...
        while DuesInvoiceRepository.token_exists(randomstring, 2019):
        ...
        if invoice_no is None:
            max_invoice_no = DuesInvoiceRepository.get_max_invoice_number(2019)
...
def dues19_listing(request):
    ...
    """
    dues19_invoices = DuesInvoiceRepository.get_all([2019])
    return {
...
def dues19_reduction(request):
    ...
    * issue a new invoice with the new amount (if new amount != 0)
    ...
    if (member is None or
        ...
        return HTTPFound(
            request.route_url('detail', member_id=member.id) + '#dues19')
